...
def trips_list():
    ...
    return jsonify([trip.to_dict(include_private=True) for trip in trips])
...
def trip_detail(trip_id):
    ...
    return jsonify(trip.to_dict(include_private=True, include_children=True))
...
def toggle_checklist_item(item_id):
    ...
    payload = request.get_json(silent=True) or {}
    item.is_done = bool(payload.get("is_done", not item.is_done))
    db.session.commit()
...
def add_note(trip_id):
    ...
    payload = request.get_json(silent=True) or {}
    title = (payload.get("title") or "Заметка").strip()
    content = (payload.get("content") or "").strip()
    if not content:
        return jsonify({"error": "Текст заметки не может быть пустым"}), 400

    note = Note(title=title, content=content, trip=trip)
    db.session.add(note)
    db.session.commit()
